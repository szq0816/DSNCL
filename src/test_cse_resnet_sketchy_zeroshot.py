...
parser.add_argument('--resume_file',
                    default='model_best.pth.tar',  # checkpoint, model_best
                    type=str, metavar='PATH',
...
def prepare_features():
    ...
    global sketchy_zero_ext
    sketchy_zero_ext = SketchyDataset(split='zero',
                                      version='all_photo',
                                      zero_version=args.zero_version,
                                      transform=transformations, aug=False)
    zero_loader_ext = DataLoader(dataset=sketchy_zero_ext, batch_size=args.batch_size,
                                 shuffle=False, num_workers=3)

    sketchy_zero = SketchyDataset(split='zero',
                                  zero_version=args.zero_version,
                                  transform=transformations, aug=False)
    zero_loader = DataLoader(dataset=sketchy_zero, batch_size=args.batch_size,
                             shuffle=False, num_workers=3)
