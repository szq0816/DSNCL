...
class SEResnetModel_KDHashing(nn.Module):
    def __init__(self, arch, num_classes, pretrained=True, freeze_features=False, ems=False):
        super(SEResnetModel_KDHashing, self).__init__()
...
class CSEResnetModel_KDHashing(nn.Module):
    ...
    def forward(self, x, y, training=False):
        if training:
            out_o, feat_list = self.original_model.features(x, y, training=training)
            out_o = self.original_model.hashing(out_o)

            hash_code = out_o
            out_cls = self.linear(out_o)
            out_kd = self.original_model.logits(out_o)

            return out_cls, out_kd, hash_code, feat_list
        else:
            out_o = self.original_model.features(x, y, training=training)
            out_o = self.original_model.hashing(out_o)
            ...
            return out_cls, out_kd, hash_code
