...
class TUBerlinDataset(Dataset):
    def __init__(self, split='train',
                 root_dir='/home/zgy/Data/dataset/TUBerlin/',
                 version='png_ready', zero_version='zeroshot',
                 cid_mask=False, transform=None, contrastive_transform=None, aug=False, shuffle=False,
                 first_n_debug=9999999):
        ...
        file_dict_semantic = os.path.join(self.root_dir, zero_version, 'semantic_gwv_dict.pkl')
        ...
        self.names_image = np.array([' '.join(ff.strip().split()[:-1]).split('/')[-2] for ff in file_content])
    ...
    def __getitem__(self, idx):
        ...
        if self.cid_mask:
            ...
            if self.contrastive_transform is not None:
                return img, img1, label, mask, wv
            else:
                return img, label, mask, wv

        else:
            if self.contrastive_transform is not None:
                return img, img1, label, wv
            else:
                return img, label, wv
