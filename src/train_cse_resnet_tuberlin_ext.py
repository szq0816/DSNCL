...
def main():
    ...
    tuberlin_train = TUBerlinDataset(split='train',
                                     version='png_ready', zero_version=args.zero_version,
                                     transform=transformations,
                                     aug=True, cid_mask=True,
                                     contrastive_transform=contrastive_transform)
    train_loader = DataLoader(dataset=tuberlin_train, batch_size=args.batch_size // 10,
                              shuffle=True, num_workers=3)

    tuberlin_train_ext = TUBerlinDataset(split='train',
                                         version='ImageResized_ready', zero_version=args.zero_version,
                                         transform=transformations,
                                         aug=True, cid_mask=True,
                                         contrastive_transform=contrastive_transform)
    train_loader_ext = DataLoader(dataset=tuberlin_train_ext, batch_size=args.batch_size // 10 * 9,
                                  shuffle=True, num_workers=3)

    tuberlin_val = TUBerlinDataset(split='val',
                                   version='png_ready', zero_version=args.zero_version,
                                   transform=transformations_val,
                                   aug=False, cid_mask=False)
    val_loader = DataLoader(dataset=tuberlin_val, batch_size=args.batch_size,
