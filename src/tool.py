...
def validate_paired(val_loader, model, criterion, criterion_kd, model_t, test_num=300):
    ...
    for i, (sketch, image, target) in enumerate(val_loader):
        ...
        # compute output
        with torch.no_grad():
            output, output_kd, hash_code = model(torch.cat([sketch, image], dim=0),
                                                 torch.cat([torch.zeros(sketch.size()[0], 1),
                                                            torch.ones(image.size()[0], 1)], dim=0).cuda()
                                                 )
    ...
    scores = -cdist(binary_sketch_hash, binary_image_hash, metric='hamming')  # 64
    # scores = -cdist(sketch_hash_all, image_hash_all, metric='cosine') # 512
